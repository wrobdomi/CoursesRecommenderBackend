...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<RatingDto> getUsersRatings(String userId) {
        UserEntity ue = userRepository.findByUserId(userId);

        List<RatingDto> rd = new ArrayList<>();

        ModelMapper modelMapper = new ModelMapper();

        System.out.println("UserService:getUserRatings - user ratings are:");
        for(UserCourseRatingEntity ucre : ue.getUsersRatings()){
            System.out.println(ucre.getRating() + " " + ucre.getCourseId() + " " + ucre.getUserId());

            RatingDto ratingDto = new RatingDto();

            CourseDto courseDto = courseService.getCourseByLongId(ucre.getCourseId());

            ratingDto.setCourseId(courseDto.getCourseId());
            ratingDto.setUserId(userId);
            ratingDto.setRating(ucre.getRating());
